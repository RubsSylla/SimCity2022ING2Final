...
void lancementPartie(BITMAP* buffer) {
    ...
    while(!key[KEY_ESC])
    {
        ...
        blit(deuxiemeFond,buffer,0,0,0,0,1024,678);
        masked_stretch_blit(capitalisme,buffer,0,0,60,35,350,300,200,110);
        ...
        if((mouse_x > 350 && mouse_x < 550) && (mouse_y > 300 && mouse_y < 435))
        {
            if(mouse_b & 1)
            {
                    ///fonction jeu mode capitaliste
                    printf("capit\n");
                    Jeu(0,1,0);
            }
        }

        if((mouse_x > 580 && mouse_x < 730) && (mouse_y > 300 && mouse_y < 435))
        {
            if(mouse_b & 1)
            {
                    ///fonction jeu mode communiste
                    printf("comm\n");
                    Jeu(0, 0, 1);
            }
        }
