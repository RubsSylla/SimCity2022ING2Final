...
void Jeu(int sauv, int cap, int com) {
    ...
    *cote_affichage = 1;
    ...
    while (!key[KEY_ESC]) {
        ...
        Ville = test_capacite_Production_elec(J,Ville);

        if(key[KEY_H])
        {
            textprintf_centre_ex(buffer, font, 512, 0, makecol(255, 255, 255), makecol(0, 0, 0), "X : %d | Y : %d", souris.x / 20,souris.y / 20);
        }
        else
        {
            textprintf_centre_ex(buffer, font, 512, 0, makecol(255, 255, 255), makecol(0, 0, 0), "G pour afficher la grille  S pour Sauvegarder  O pour mettre le jeu en Pause");
        }


        if (*cote_affichage == 1) {
            if (compteur < 60) {
                textprintf_ex(buffer, font, 0 + 20, 20 + 20 + 10, makecol(255, 255, 255), -1, "Temps ecoule : %d",
                              compteur);
            }
            if (compteur >= 60) {
                min = compteur / 60;
                ...
                textprintf_ex(buffer, font, 0 + 20, 20 + 20 + 10, makecol(255, 255, 255), -1, "Temps ecoule : %d:%d",
                              min, compteur);
            }
        ...
        if (*cote_affichage == 2) {
            if (compteur < 60) {
                textprintf_ex(buffer, font, 0 + 20 + 774, 20 + 20 + 10, makecol(255, 255, 255), -1, "Temps ecoule : %d",
                              compteur);
            }
            if (compteur >= 60) {
                min = compteur / 60;
                ...
                textprintf_ex(buffer, font, 0 + 20 + 774, 20 + 20 + 10, makecol(255, 255, 255), -1,
                              "Temps ecoule : %d:%d", min, compteur);
            }
